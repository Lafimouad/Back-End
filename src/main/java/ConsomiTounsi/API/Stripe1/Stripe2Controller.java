...
import ConsomiTounsi.API.Stripe.CheckoutPayment;
...
@CrossOrigin("*")
@RestController
@RequestMapping(value = "/apidonate")
public class Stripe2Controller {
     ...
     */
    public String paymentWithCheckoutPage(@RequestBody Donation2 payment) throws StripeException, MessagingException {
       ...
       // Pool p = Poor.findById(1L).orElse(new Pool());
        this.pl.AddamountTopool(payment.getAmount());
        Donation payment1 = new Donation();
        payment1.setDate(now);
        payment1.setAmount_donation(payment.getAmount());
        payment1.setEmailAdress(payment.getEmail());
